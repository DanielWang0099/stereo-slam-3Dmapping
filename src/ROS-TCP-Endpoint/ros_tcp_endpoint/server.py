...
class SysCommands:
    ...
    def start_gate_navigation(self):
        """Start gate navigation process via Unity command"""
        try:
            # Send command to navigation controller
            nav_command_msg = String()
            nav_command_msg.data = "START"
            
            # Create publisher if it doesn't exist
            if not hasattr(self.tcp_server, 'nav_command_pub'):
                self.tcp_server.nav_command_pub = self.tcp_server.create_publisher(
                    String, '/unity/navigation_commands', 10
                )
            
            self.tcp_server.nav_command_pub.publish(nav_command_msg)
            self.tcp_server.loginfo("Gate navigation start command sent")
            
            # Send confirmation to Unity
            response = {"status": "command_sent", "action": "start_navigation"}
            self.tcp_server.send_unity_message("gate_navigation_response", 
                                               json.dumps(response).encode())
            
        except Exception as e:
            self.tcp_server.logerr(f"Failed to start gate navigation: {e}")
            error_response = {"status": "error", "message": str(e)}
            self.tcp_server.send_unity_message("gate_navigation_response",
                                               json.dumps(error_response).encode())

    def stop_gate_navigation(self):
        """Stop gate navigation process via Unity command"""
        try:
            # Send command to navigation controller
            nav_command_msg = String()
            nav_command_msg.data = "STOP"
            ...
            self.tcp_server.nav_command_pub.publish(nav_command_msg)
            self.tcp_server.loginfo("Gate navigation stop command sent")
            
            # Send confirmation to Unity
            response = {"status": "command_sent", "action": "stop_navigation"}
            self.tcp_server.send_unity_message("gate_navigation_response",
                                               json.dumps(response).encode())
            
        except Exception as e:
            self.tcp_server.logerr(f"Failed to stop gate navigation: {e}")
            error_response = {"status": "error", "message": str(e)}
            self.tcp_server.send_unity_message("gate_navigation_response",
                                               json.dumps(error_response).encode())

    def get_gate_navigation_status(self):
        """Get current gate navigation status"""
        try:
            # Send status request to navigation controller
            nav_command_msg = String()
            nav_command_msg.data = "STATUS"
            ...
            self.tcp_server.nav_command_pub.publish(nav_command_msg)
            self.tcp_server.loginfo("Gate navigation status requested")
            
        except Exception as e:
            self.tcp_server.logerr(f"Failed to get gate navigation status: {e}")
            error_response = {"status": "error", "message": str(e)}
            self.tcp_server.send_unity_message("gate_navigation_status",
                                               json.dumps(error_response).encode())
